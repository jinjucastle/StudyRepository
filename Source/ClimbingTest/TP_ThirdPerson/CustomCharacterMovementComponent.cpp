...
namespace TestCharacter {
	...
	FAutoConsoleVariableRef CVar_GroundTraceDistance(TEXT("TestCharacter.GroundTraceDistance"), GroundTraceDistance, TEXT("Distance to trace down when gennerating ground information."), ECVF_Cheat);

}


const FTestCharacterGroundInfo& UCustomCharacterMovementComponent::GetGroundInfo()
{
	...
	else
	{
		...
		const float CapculeHalfHeight = CapsuleComp->GetUnscaledCapsuleHalfHeight();
		const ECollisionChannel CollisionChannel = (UpdatedComponent ? UpdatedComponent->GetCollisionObjectType() : ECC_Pawn);
		const FVector TraceStart(GetActorLocation());
		const FVector TraceEnd(TraceStart.X, TraceStart.Y, (TraceStart.Z - TestCharacter::GroundTraceDistance - CapculeHalfHeight));
		
		FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(TestCharacterMovementcomponent_GetGroundInfo), false, CharacterOwner);
		...
		CachedGroundInfo.GroundDistance = TestCharacter::GroundTraceDistance;
		...
		{
			CachedGroundInfo.GroundDistance = FMath::Max((HitResult.Distance - CapculeHalfHeight), 0.0f);
		}
	}
